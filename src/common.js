...
/**
 * Позначити піддерево як частину модуля (`userData.mod`).
 *
 * Групи в сцені змішані за призначенням: у вузлі барабана сидять і барабанне
 * колесо (передача), і пружина (енергія), і маточинне колесо диференціала
 * (запас ходу). Тому приналежність визначається не батьківською групою, а
 * тим, ХТО деталь створив — кожен модуль позначає своє одразу після збірки.
 *
 * Перший тег виграє: модуль, що додає деталі в чужу групу пізніше, не перетирає
 * вже позначене.
 */
...
/**
 * Позначити, ЯКИЙ саме варіант стоїть у гнізді.
 *
 * Модуль (`mod`) — це гніздо: підсвітка, камера й розріз звертаються до нього
 * й не хочуть знати, що всередині. Варіант потрібен лише тим, хто працює з
 * самою заміною: показати/сховати, порахувати деталі.
 */
