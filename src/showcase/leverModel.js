...
/**
 * Колесо з клубовими зубцями швейцарського ходу: тонкі вигнуті клини
 * (club fins) з нахилом за ходом, широкі западини між ними; запірна грань —
 * увігнута дуга, імпульсна — випукла дуга до наступного вістря. Профіль як
 * у серійних анкерних коліс (пор. демонстраційні моделі ходу — вигнуті
 * тонкі зубці-кігті, а не трикутна пилка).
...
function makeClubWheel(material) {
  ...
  for (let i = 0; i < TEETH; i++) {
    ...
    // Запірна грань — увігнута дуга: з вістря згинається до кореня.
    shape.quadraticCurveTo(
      ...polar(WHEEL_R * 0.6, a - 0.02 * step),
      ...polar(WHEEL_ROOT, a - 0.07 * step)
    );
    // Дно глибокої западини.
    shape.lineTo(...polar(WHEEL_ROOT * 0.82, a - 0.24 * step));
    // Імпульсна грань — випукла дуга до наступного вістря.
    shape.quadraticCurveTo(
      ...polar(WHEEL_R * 0.93, a + 0.5 * step),
      ...polar(WHEEL_R, a + step)
    );
  }
  shape.closePath();
